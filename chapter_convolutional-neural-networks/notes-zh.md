...
$$H=W=28,\ k_h=k_w=3,\ p_h=p_w=1,\ s_h=s_w=1 \Rightarrow H_{\text{out}}=W_{\text{out}}=28 \tag{3.1a}$$
> 这是最常见的 “same” 卷积：输入 $28\times28$，核 $3\times3$，上下左右各 padding 1，步幅 1。代入 (3.1)：$H_{\text{out}}=\left\lfloor(28+2\cdot1-3)/1\right\rfloor+1=28$，宽同理，所以输出尺寸不变。

$$H=W=28,\ k_h=k_w=3,\ p_h=p_w=1,\ s_h=s_w=2 \Rightarrow H_{\text{out}}=W_{\text{out}}=14 \tag{3.1b}$$
> 同样是 $3\times3$ 核和 padding 1，但把步幅改成 2，相当于“每次跳两格”做卷积。代入 (3.1)：$H_{\text{out}}=\left\lfloor(28+2-3)/2\right\rfloor+1=\left\lfloor27/2\right\rfloor+1=14$，宽同理，所以输出变成 $14\times14$（下采样）。

$$H=W=9,\ k_h=k_w=3,\ p_h=p_w=0,\ s_h=s_w=1 \Rightarrow H_{\text{out}}=W_{\text{out}}=7 \tag{3.1c}$$
> 这是 “valid” 卷积：不 padding 时，输出会变小。代入 (3.1)：$H_{\text{out}}=\left\lfloor(9+0-3)/1\right\rfloor+1=7$，宽同理，所以 $9\times9 \to 7\times7$，这也对应你前面推过的 9→7。
...
汇聚层就是干这个的：它在一个小窗口内取统计量（最大值或平均值），把精确的位置信息"模糊化"。
